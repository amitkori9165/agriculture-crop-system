...
//	For finding the crop by their name...
	...
	public List<Crop> getCropByName(@Valid String name);
...
//	For deleting the crop by name....
	...
	public int deleteCropByName(@Valid String name);
	
//	@Modifying
//	@Transactional
//	@Query(value = "DELETE FROM crop_details WHERE booking_id = ?1",nativeQuery = true)
//	public int deleteCropsByBookingId(@Valid int bookingId);
	
	@Query(value = "SELECT * FROM crop_details WHERE booking_id = ?1", nativeQuery = true)
	public Crop getCropByBookingId(@Valid int bookingId);
